...
import { Book, BooksByCategory } from '../../models/model';
...
constructor(private as: ApiService, private sb: MatSnackBar){
  as.getBooks().subscribe({
    next: (res: Book[]) =>{
      this.books = [];
...
orderBook(book: Book) {
  this.as.orderBook(book).subscribe({
    next: (res) => {
      if (res === 'ordered') {
        book.ordered = true;
        let today = new Date();
